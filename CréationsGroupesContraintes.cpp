...
void CreationGroupesContraintes(Bibliotheque biblio, Liste<Contraintes>& contraintes) {
	...
	do {
		// premi\212re s\202lection
		options.ListeElements[options.nbElements] = createTitre("Selection premi\212re contrainte");
		...
		if (select1 != options.nbElements) {
			...
			do {
				// s\202lection de l'op\202rateur
